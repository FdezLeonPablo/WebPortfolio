...
import CloseMenu from '@/app/assets/close-line.svg';
  ...
  return (
    <nav className="p-4 bg-primary-100">
      {folderStructure.map((folder) => (
        <div key={folder.name} onClick={() => toggleFolder(folder.name)} className="cursor-pointer">
          <span className="flex items-center text-sm text-secondary-100 mb-1">
            ...
            <FolderIcon className={`w-4 h-4 mr-2 ${folderColors[folder.name]}`} />
            {folder.name}
          </span>
